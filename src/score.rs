...
use nature_common::{Instance, NatureError, Result, SEPARATOR_INS_PARA};
...
pub extern fn name_to_id(para: &Vec<Instance>) -> Result<Vec<Instance>> {
    ...
    for input in para {
        ...
        let part: Vec<&str> = one.para.split(&*SEPARATOR_INS_PARA).collect();
        ...
        match option {
            ...
            Some(id) => one.para = id.to_owned().to_owned() + &*SEPARATOR_INS_PARA + part[2]
        }
        rtn.push(one);
    }
    Ok(rtn)
